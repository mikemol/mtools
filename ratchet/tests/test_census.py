...
def test_the_key_keeps_the_file_rather_than_collapsing_to_the_rule() -> None:
    ...
    assert keys == frozenset(
        {"src/a.py:compare-to-empty-string", "src/b.py:compare-to-empty-string"}
    )
...
def test_an_unexpected_exit_refuses_rather_than_reporting_an_empty_census(tmp_path: Path) -> None:
...
def test_a_synthesized_package_marker_is_not_censused(tmp_path: Path) -> None:
    ...
    keys = parse_concise(
        ["pkg/__init__.py:1:1: some-rule: msg", "real/__init__.py:1:1: some-rule: msg"], tmp_path
    )
...
def test_a_synthesized_bootstrap_is_not_censused(tmp_path: Path) -> None:
    ...
    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "_private.py").write_text(
        '"""A real private module."""\n', encoding="utf-8"
    )
    keys = parse_concise(
        [
