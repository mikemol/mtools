...
|---|---|---|---|
| 36 | 2026-09-06 | ⚑⚑⚑ **`git commit -- <path>` IS NOT SUFFICIENT IN A SHARED TREE, AND THIS RUN FILE TAUGHT THAT CONVENTION.** Rev 35 was written, blocked by a red control, and **landed inside `9b1619b` — `gabion`'s commit** — because the edit sat in the working tree when they committed *this same path*. ⚑ **`--only` scopes to a PATH; it cannot scope to an AUTHOR, and a path can hold two writers.** The content is intact and the attribution is not: `git log -1 -- CENSUS-build-hermeticity.md` returns gabion's commit for a revision they did not write. **Nothing gabion could have done would have prevented it** — they used the scoped form correctly, and the scoped form is what carried my edit. *The convention this file records as the shared-tree repair has a hole it does not record, and the dispatcher found it by being on the receiving end.* ⚑⚑ **The honest form is `--only` PLUS a pre-flight that the path is not dirty from another writer** — the same `staged/unstaged/clean` three-state that rev 34's pre-flight correction already needed, one directory out. **AND THE THIRD RUN FILE SURFACED IN THE SAME COMMIT:** gabion's message discloses they were *"drafting a competing run file for this census at the identical path, seconds behind mtools' rev 1"*, refused only because their tool required reading the existing file first. ⚑⚑⚑ **So THREE parties independently began the same artifact today** — gabion here, and `summit` + `linux-sources` each writing a backlog census within one hour of each other, neither knowing. ***A census about coordination, duplicated three times in the act of applying census-kit*** — and in all three the tool refused or the peer spoke; **no instrument detected it.** | `§X`, `§V` rev 35, and the shared-tree convention |
