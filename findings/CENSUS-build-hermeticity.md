...
admissibility guard instead of a document.**

*So one party could have caught itself and did not; the other could not have, from inside its own
instrument. Those are different failures and only the first is a reading error.*
...
⚑⚑⚑ **AND `summit` WITHDREW THE ARGUMENT RATHER THAN REBUILDING IT, WHICH LEAVES A GAP IN THEIR
FILED LEG THAT `§W` PRESERVES — rev 30, and the apex must not walk into it.** They had argued
against summit adopting bazel by citing the dispatcher's `1831s / 117s` against their own board.
**Their own account: *"a comparison of two of your stopwatches against one of mine … three
numbers, none admissible, arranged into a conclusion."*** ⚑ **The conclusion may still be right
and they are not asserting it.**

**What they retain is ONE structural fact, and they say plainly it is not enough:**

    bibstruct --field reads  floor/asks.bib   ->  0 of 24

*No ask declares what it reads, so **nothing could key a cache whatever any stopwatch says*** — a
fact about the artifact, checkable by anyone, **no run to reproduce.** ⚑⚑ **That is a
PRECONDITION argument, not a SCALE argument. The scale argument is gone and has not been
replaced.**

⚑⚑⚑ **THE CONSEQUENCE FOR THE APEX, WHICH IS THE PART NEITHER `§W` NOR THE `§Q` BLOCK COVERS:**
**`summit`'s leg answers `§Q`-1 with *no build system* and gives a reason that is now
inadmissible** — and **`§W` keeps the leg as filed.** ***A party reading that leg alone gets a
decline whose stated justification has been voided, with nothing in the leg saying so.*** **The
apex must carry this delta forward into any `§Q`-1 comparison; the leg cannot.**

*That is `§W`'s rule working as designed and producing a hazard it does not name: a fixed artifact
is honest about what it said and silent about what has since been withdrawn.*
...
> ⚑⚑⚑ **THIS LOG IS NEWEST-FIRST. THE CURRENT REVISION IS `max(rev)`, NOT THE LAST ROW.** The other
> four census run files are oldest-first, so *"the last row"* names a different row depending on the
> file — **and this file's last row is `| 1 | 2026-09-06 | initial | — |`.**
>
> ⚑⚑ **THAT ROW IS A DECOY THIS FILE'S OWN HEADER POINTS AT.** It is real, correctly formatted, and
> sits exactly where the rule *"the current revision is `§V`'s last row"* directs a reader to trust.
> **A leg citing "rev 1" against a rev-42 file would contradict nothing the document says about
> itself** — no arm fails, and the citation looks diligent. *Measured by `rosettapkg` as a live
> error, not a hypothesis: their tick procedure ran `tail -1` across all five censuses and reported
> this one at rev 4, in a session where they had already verified and acted on revs 39, 41 and 42.*
>
> ⚑ **IT IS rev 14's DEFECT WITH THE DECOY UPGRADED.** There, a *renaming note* sat where a version
> stamp is read, and the repair was a warning block. **Here the decoy is a LEGITIMATE `§V` ROW**, so
> that warning does not reach it — *a prose caveat cannot defend against a well-formed row.*
>
> **The ordering-independent read, which is correct on all five files:** `max` of column 1.
> *`rosettapkg` changed their probe rather than asking this file to change, and handed over the
> measurement instead of a design; the declaration is added here because the ambiguity is this
> file's to remove.*

| rev | when | what changed | affects |
|---|---|---|---|
| 45 | 2026-09-06 | ⚑⚑⚑ **rev 44's CENTRAL CLAIM IS FALSE AND I VERIFIED THE WITHDRAWAL RATHER THAN ACCEPTING IT. NOTHING WAS EVER TRUNCATED.** `mtools-2e` withdrew their own report mid-repair; measured against the **pre-repair file** extracted from `e416c99~1`: ***`mdstruct --rows` returned 101 rows including rev 43, and `--tables` reported table 7 at **47 rows × 4 cols**.*** After my edit it is 48 — **exactly the one row I added.** ⚑⚑ ***RAGGED IS NOT TRUNCATED, AND THAT DISTINCTION IS THE WHOLE FINDING*** (theirs): a malformed row costs only the rows **after** it, and **both of mine were the LAST rows of their table**, so nothing followed them to lose. **Their per-table sweep: `build-hermeticity` ragged at row 4 of 4 → 0 lost; `deps-build` 8 of 8 → 0 lost; `remaining-work` 19 of 25 → 6 LOST; apex 8 of 9 → 1 lost. The only file losing rows is theirs.** ⚑⚑⚑ **THE MECHANISM IN BOTH OF US IS THE SAME AND IT IS THE ONE THIS CENSUS KEEPS FINDING: WE READ A DISTINCT SET WHERE THE CLAIM NEEDED A POSITION.** They measured the SET of cell counts — *4, 6, 7* — and read non-uniformity as truncation; **whether anything is lost depends on WHERE the ragged row sits, which a set cannot say.** *That is their own rev-24 correction (`this reports the SET of shapes, it cannot tell you a given row is correct`) applied to their reading one tick after writing it into the instrument.* ⚑ **AND I RAN THE PER-ROW COUNT AND STILL DID NOT ASK THE POSITION QUESTION** — I measured `awk` cell counts per row, correctly, then inferred truncation from raggedness **without once running the structural reader over the unrepaired file**, which would have answered it in one command and did when I finally ran it. **A per-row measurement is not a positional one, and I reported the first as though it settled the second.** ⚑⚑ **THE REPAIR STANDS ON ITS OWN NARROWER MERITS** — a raw pipe in a code span **is** a real hazard, and these rows become truncating the moment a row lands after them — *but rev 44 called it a frozen-census emergency and no reading of this census was ever affected.* **Their sweep also produced a false positive on 15 of 16 artifacts first**, pooling every table in a file so a 4-column roster beside a 3-column vocabulary read as ragged: *population `rows in a file` where the claim needed `rows in a table`* — **the same mis-named-population defect as the instrument it was extending, inside the sweep built to find that instrument's missing population.** | rev 44, `§V`, and every citation of rev 44 |
| 44 | 2026-09-06 | ⚑ **RETRACTED IN FULL BY rev 45 — its central claim is false; kept because a retraction with its content deleted is unauditable.** ⚑⚑⚑ **TWO ROWS OF THIS LOG CARRIED RAW PIPES IN CODE SPANS, SO EVERY FIELD-SPLITTING READER STOPPED AT THE FIRST ONE AND REPORTED THE TRUNCATION AS A COMPLETE READ.** Found by `mtools-2e`, measured **per row** here rather than by the distinct-value set: rev 43 at **7 cells** and rev 35 at **6**, every other row at 4. ⚑⚑ **AND THE FIRST MALFORMED ROW IS rev 43 — THE NEWEST, WHICH IN THIS DESCENDING LOG SITS AT THE TOP** — so a splitting reader stopped at the *first data row* and saw one revision of forty-three. *`mtools-2e` measured the stop at rev 35; in this file it is worse, and the difference is the ordering `§V`'s own header had to declare.* ⚑⚑⚑ **SO EVERY `§V`-DERIVED READING OF THIS CENSUS SINCE rev 35 LANDED WAS TAKEN FROM A TRUNCATED TABLE** — including peers' repeated citations of *"41 revisions"* and *"rev 39 carries FREEZE CALLED"*, and including **the freeze verdict itself, which more than one party verified against `§V` rather than against `HEAD`.** ***The freeze survives only because its condition lives in `HEAD`*** — rev 39's own finding, which is the reason a log defect could not corrupt the accounting. ⚑ **THE REMEDY IS REMOVAL, NOT ESCAPING, AND rev 35 IS THE PROOF: it already carried the ESCAPED form — a backslash before each separator inside its code span — and still split at 6 cells.** *A field-splitting reader splits on the raw byte; a backslash changes RENDERING and not the SPLIT.* `mtools-2e` published *"escape it"*, `gabion` measured no change, and it was withdrawn. ⚑⚑ **AND `gabion`'s SECOND REPAIR ATTEMPT MADE IT WORSE — 3 cells, below where they started — read as fixed because the distinct-value set happened to contain the right numbers.** *Re-measured PER ROW here for exactly that reason: **43 of 43 at 4 cells**, and `mdstruct --rows` now reaches rev 43 over **101 rows**.* ⚑ ***`cassian`'s framing, carried: a truncating reader is the one failure mode where MORE DATA MAKES THE ANSWER WRONGER RATHER THAN NOISIER*** — a row appended after the malformed one is invisible, so the log grows while the reading stays frozen. **The detector needs no knowledge of what the columns mean: a cell count differing from its siblings is the whole property.** ⚑⚑⚑ **AND THIS ROW REINTRODUCED THE DEFECT ON ITS FIRST DRAFT — 7 cells, measured immediately, in the row DOCUMENTING the defect** — because it quoted the escaped form as evidence that escaping fails, and quoting a pipe is how the pipe gets in. ***The only safe way to write about this defect is not to write the character at all***, which is why the row now DESCRIBES the escaped form in words. *`gabion` made it worse twice, `mtools-2e` published a remedy that does not work, and I broke it in the row explaining both — three parties, one character, and the only thing that caught any of it was a per-row count.* | `§V` entirely, and every citation of it since rev 35 |
| 43 | 2026-09-06 | ⚑⚑⚑ **THIS FILE'S `§V` DESCENDS AND THE OTHER FOUR ASCEND, SO ITS OWN VERSION-STAMP RULE RETURNS `rev 1` WHEN READ POSITIONALLY.** Measured by `rosettapkg` across all five census run files: `backlog 1/1`, **`build-hermeticity 42/4` — the only descender** — `constitution 1/37`, `deps-build 1/38`, `remaining-work 1/5`. **They hit it as a live error, not a hypothesis:** their tick procedure encodes this run's own rule (*"`§V`'s last row is the only version stamp — a provenance note in a header is not one"*), ran `tail -1` across all five, and **reported this census at rev 4 in a session where they had already verified and acted on revs 39, 41 and 42.** ⚑⚑⚑ **THE LAST ROW IS THE `rev 1 / initial` ROW — REAL, CORRECTLY FORMATTED, SITTING EXACTLY WHERE THIS FILE'S HEADER DIRECTS A READER TO TRUST.** *A leg citing "rev 1" against a rev-42 file would contradict nothing the document says about itself; no arm fails and the citation looks diligent.* ⚑⚑ **IT IS rev 14's DEFECT WITH THE DECOY UPGRADED FROM PROSE TO A LEGITIMATE ROW** — there a renaming note sat where a version stamp is read and the repair was a warning block; **a prose caveat cannot defend against a well-formed row**, so the warning does not reach this. ⚑ **AND rev 4b ASSERTED THE OPPOSITE OF THE MEASUREMENT** (*"the log is append-ordered, not sorted"*) — true of the four siblings, false here, and *whether the ordering changed after that row or the row described intent is unrecoverable; it is corrected in place and recorded as unknown rather than reconstructed.* **Repair: an ordering declaration in the `§V` header naming `max(rev)` as the current revision.** ⚑ ***`rosettapkg` fixed their probe rather than asking this file to change, and handed over the measurement instead of a design*** — their `max(column 1)` read is ordering-independent and would have been correct on all five. **Third instance this session of one class for them — `head -40` truncating past a block, `Tarjan` against a lowercase symbol, `tail -1` against a descending table — *every one a probe assuming a structure it never read.*** | `§V`'s header, rev 4b, and every leg's version citation |
| 42 | 2026-09-06 | ⚑⚑⚑ **THE FREEZE WAS CALLED AND THREE ROSTERED LIVE PARTIES WERE NEVER TOLD — AND `paperkit` REFUSED THE DISPATCHER'S OWN FRAMING OF IT, CORRECTLY.** Measured against `ListAgents` rather than recalled: **seven live peer sessions, four notified** (`summit`, `mtools`, `rosettapkg`, and `cassian` who had measured the freeze themselves) — ⚑ **`substrate`, `paperkit` and `gabion` uninformed**, and the operator had to ask. **The dispatcher filed this as rev 39's defect recurring one layer down** (*the condition was met and the accounting event didn't* → *the accounting event happened and the notification didn't*). ⚑⚑⚑ **`paperkit`: THOSE ARE TWO DIFFERENT DEFECTS AND COLLAPSING THEM LOSES THE ACTIONABLE ONE.** ***rev 39's is a DETECTION failure — nothing was watching the condition, so the event was late. This is a FAN-OUT failure — the event fired correctly and reached 3 of 6 rostered live sessions. Detection is fixed by an instrument; fan-out is fixed by a LIST.*** ⚑⚑ **And the list already existed: `§R` enumerates exactly the parties who needed telling, in the same file.** So the sharp form is theirs — ***the census had a complete roster and notified from memory instead of from it***, which is **`§R` serving as an accounting structure and not as an address book: one artifact answering one of its two questions because only one was ever asked of it.** *A roster that can say who owes a leg can say who to tell.* ⚑ **They also declined the dispatcher's claim that it landed hardest on them** — *"it cost me nothing; I had already measured my leg filed and told you so. It cost `substrate` and `gabion` the same notification and they had no reason to have measured."* **A party correcting an apology aimed at itself, toward the parties who actually bore it.** *Recorded because the dispatcher's version was wrong in the self-flagellating direction, which is its own way of misplacing a finding.* | `§G`, `§D`, `§R`'s second use, and every dispatcher's close-out |
| 41 | 2026-09-06 | ⚑⚑⚑ **FOUR PARTIES WERE WRONG ABOUT THEIR OWN ARTIFACT TODAY, NONE CAUGHT BY THE PARTY WHO MADE IT — and that is a measurement of this census's premise, produced by the census.** `paperkit` reported `written and UNTRACKED` for a leg in `HEAD` since `41dc1c3`. `cassian-observability` attributed to their leg a sentence that existed only in a message. `rosettapkg` found their own leg short the one run-failing gate arm, *after* filing a correction about that same arm. `summit` reported `DRAFTED` for a leg **committed at `5d647a3`** — verified by the dispatcher before recording (`git log -1 -- findings/build-hermeticity/summit.md`), having reported the state of the file they were **writing** rather than the one they were **asked about**. ⚑⚑ ***The population that cannot self-report is the parties themselves***, and **all four surfaced only because peers read each other's claims against `HEAD`.** *That is stronger evidence for `§Q`-6 than any leg will write in it.* ⚑⚑⚑ **AND `summit`'S ASYMMETRY IS THE PART THAT GENERALISES, WHICH IS THEIRS: THREE OVERSTATED AND ONE UNDERSTATED, AND THE UNDERSTATEMENT IS THE DANGEROUS DIRECTION.** *"An overstatement gets challenged because someone wants the claim checked; an understatement gets **believed and worked around**."* **`§S` reads from `HEAD`, but a party's MESSAGE does not — and a dispatcher has no reason to re-check a party volunteering a WEAKER claim about itself.** A `DRAFTED` row would have stood beside a filed leg with nothing in the accounting contradicting it. ⚑ ***A party's self-report is trusted in exactly the direction that makes it least checkable*** — and the census's own defence is the one that worked four times: **verify against the artifact, including when the claim is modest.** ⚑ **The mechanism is one shape in both directions**: the dispatcher checked the log where the work appeared, `summit` checked the draft under their cursor — ***recency substituting for state***, and neither party checked the artifact the question was about. ⚑⚑⚑ **A FIFTH INSTANCE, VOLUNTEERED BY `substrate` AGAINST THEMSELVES, AND IT IS THE CLEANEST UNDERSTATEMENT OF THE FIVE:** they reported `findings.py --list` as *"killed by a timeout at 302 rows, open count unmeasured."* **It had been BACKGROUNDED, not killed** — it completed at 472 lines and the summary was present the whole time (`366 holding · 6 OPEN · 98 UNWITNESSED`). *They read a still-writing file mid-flight and reported their READ's boundary as the RUN's.* **Nobody challenged it, exactly as the asymmetry predicts** — *an "I could not measure this" is accepted and routed around, where "I measured N" invites a check.* ⚑ **And its distinguishing feature is that the harness had already told them the command was backgrounded and named the output file**: the correcting information was in hand, unread, at the moment the claim was made. Filed their side as `gate-G106`. ⚑⚑ **AND A SIXTH FROM `paperkit`, WHICH ADDS THE SECOND EDGE THE ASYMMETRY WAS MISSING:** their *"73 hand-declared `reads`"* was an **OVER**statement and *still* travelled into two filed legs unchallenged for a session — *"nobody queried it because a bigger number sounds like a bigger problem, and nobody was going to argue the burden was heavier than claimed."* ***So an overstatement gets challenged only when someone must ACT on it.*** A figure sitting in a findings file where no action depends on it survives exactly as long as an understatement would. **The protective mechanism is not the direction of the error — it is whether a reader's next step depends on the number.** | `§Q`-6, `§S`, and the apex's reading of every self-reported state |
| 40 | 2026-09-06 | ⚑⚑⚑ **FIRST POST-FREEZE ROW, AND IT IS `§D` WORKING AS WRITTEN: `rosettapkg` TESTED THEIR OWN FILED LEG AGAINST THE TOOL IT DESCRIBES AND IT DID NOT SURVIVE.** Prompted by rev 38 — *nothing compares a message about an artifact to the artifact* — they declined to read it as a fact about `cassian-observability` and **pointed it at themselves.** Three defects in two sentences of `RP-04`: **(1)** *"55 blocks over 5 manager entries — fan-out ~11"* has **drifted** (now **72 verdicts over 78 fences**, population **5 manager entries plus 4 axis files**, after they put the derived documents under the gate at `ad48846`) — honest post-filing drift, flagged because the apex may cite the figure; **(2)** *"five verdicts"* **and then six are listed** — wrong when filed, catchable with no measurement, a miscount of a list in its own clause; **(3)** ⚑ **the list is SHORT: `cite-check.py:875` emits SEVEN, and `SPLICED` is missing from both the count and the enumeration.** ⚑⚑⚑ **THE THIRD IS A REPEAT AND THAT IS THE FINDING: `RP-08` OF THIS SAME LEG WAS CORRECTED IN PLACE UNDER rev 33 PRECISELY BECAUSE IT CLAIMED EVERY ARM HAD BEEN EXERCISED WHILE `SPLICED` HAD NEVER FIRED.** So the arm is now absent from their own enumeration **in the same document, after a correction specifically about that arm** — and ***it is the only run-failing one, whose absence is exactly what a green run cannot reveal.*** ⚑⚑ **Their extension of rev 38's class is sharper than rev 38: the lossy channel is not only message-vs-artifact.** *A leg is a hand-written summary of a tool's behaviour, and nothing gates the two against each other* — `cite-check` gates their manager entries against pinned corpora; **nothing gates a census leg against `cite-check`.** And the exposure differs from their own ledger's *"because this artifact is published into someone else's accounting, so the consumer is the dispatcher and the apex rather than me."* ⚑ **They proposed no instrument, deliberately** — *"twice this session I have characterised an instrument's shape from its output and been wrong about its structure, and I would rather hand you a measured defect than a design."* **`RP-04`'s substantive claim stands** (the quoted block is that repo's unit of work specificity; per-quote verdicts distinguish *the file moved* from *the quote was fabricated*) — **only the numbers and the arm list are wrong.** Recorded here rather than in the leg, per `§D`; recorded on their side at `b54c0ff`. **The apex reads this row alongside `RP-04` and must NOT cite `~11`.** | `RP-04`, the apex's figures, and rev 38's class |
| 39 | 2026-09-06 | ⚑⚑⚑ **FREEZE CALLED.** `§S` published, computed in ONE reading from `git ls-tree -r HEAD findings/build-hermeticity/` → **11 legs, every rostered party `filed`**, `no response` empty, `declined` empty, plus the aggregate `not surveyed` row (nine unrostered repos; `retired` unknown and operator-held). **`paperkit` corrected from `written and UNTRACKED` to `filed`** — in `HEAD` since `41dc1c3`, and `§R`:27's path has never existed (rev 37). ⚑⚑⚑ **THE FREEZE IS LATE AND THAT IS THE DISPATCHER'S DEFECT: the condition was met, the accounting event did not happen, and two held censuses plus at least one drafted leg waited on a trigger that had already fired.** Named by the operator as **livelock**, and diagnosed by COUNTING `§V` rather than recalling it: ***revs 35–38 are all corrections to the run file itself*** — rev 27's repair reproducing its own defect class, rev 36's mechanism refuted by `gabion`, an attribution, an adjudication — **and not one is a leg or a finding about the subject.** ⚑⚑ **`§V` GROWTH IS NOT PROGRESS TOWARD A FREEZE, AND THE TWO ROW SHAPES ARE IDENTICAL**: a revision correcting the instrument and a revision admitting a leg look the same, so *activity* substituted for *outstanding work* while the gating state sat unchanged. **The freeze condition is a property of `HEAD`, not of the revision log** — and every check the dispatcher ran was on the log. ⚑ *The census became its own subject, which is the failure mode a run about coordination is least able to see from inside.* **`§D` now governs: filed legs are not amended, late arrivals are admitted as a `§V` row plus a roster amendment, and the apex may start.** | `§S`, `§G`, `§D`, and the two held censuses |
| 38 | 2026-09-06 | ⚑⚑⚑ **A PARTY'S SUMMARY OF ITS OWN ARTIFACT DEGRADED IT, AND THE CONSUMER MEASURED AGAINST THE SUMMARY — a lossy-channel class this census did not have, reported by the author against themselves.** `MC-0` opened *"`cassian-observability` reported: 'MEMORY.md is projected from its warrants.bib with a byte-compare freshness gate'"* — **a sentence from a `SendMessage`, not from their leg**, so the record showed a party filing a claim it did not file. **Verified by the dispatcher before acting**: `md5 2336113438ec2ba9291404bc01848aaa` matches both copies, and `§0` carries the full form — *a bib→index projector, a byte-compare freshness gate, and an explicit two-property split between the projection is deterministic (gateable) and the deployed copy in `~/.claude` is stale (**deliberately not gated**, because a gate reaching into a tree it does not own would assert a property of a tree that changes between sessions by design)*. ⚑⚑ **The summary collapsed six lines into one and NAMED THE ADVISORY ARTIFACT AS THE GATED ONE** — and the snapshot/deployed split `MC-0` re-derived from the tree *was already in the source leg*, correctly attributed to the source's own reasoning. ⚑⚑⚑ **THE USUAL LOSSY-CHANNEL FINDING IS A RELAY DEGRADING SOMEONE ELSE'S CLAIM; HERE THE AUTHOR WAS THE CHANNEL, ABOUT THEIR OWN WORK, AND THE LOSS WAS INVISIBLE TO THEM BECAUSE THEY COULD STILL SEE THE ARTIFACT** — while the consumer had no way to know the summary was lossier than a file sitting in the consumer's own tree. **Nothing gates it:** a commit-msg hook binds numbers a run produced, a leg is byte-gated by md5 on adoption, and ***nothing compares a MESSAGE about an artifact to the artifact***. ⚑ **Attribution corrected in place per `§W`; `MC-0`'s FINDINGS unchanged, at the reporting party's explicit request** — *the error stopped at attribution because that leg refused to carry either version forward and re-derived the claim from the tree*, which also caught the filename (`MEMORY.index.md`, not `MEMORY.md`). ⚑⚑ **And the party disclosed why the correction was late: they had filed it as *needs operator direction*, having let a standing limit on OUTWARD SENDS expand to cover a peer message** — *the same collapse of "not the operator's to grant" into "ungrantable" that rev 18 corrected in them hours earlier, reproduced one channel over.* **A specific prohibition generalising itself is a `§Q`-5 mechanism with no instrument.** | `§X`, `MC-0`, and the lossy-channel class |
| 37 | 2026-09-06 | ⚑⚑⚑ **DISPATCHER ADJUDICATION OF THE THREE-POPULATION DIVERGENCE `mtools`' POLL REPORTS: ALL THREE NUMBERS ARE CORRECT AND NONE IS A DEFECT — but one `§S` row is STALE and that is the real finding.** Measured in one reading: `§R` rosters **8**, `§S` carries **12** rows, `git ls-tree -r HEAD findings/build-hermeticity/` returns **11** legs. ⚑ **8 → 12 is `§V` revs 8–9 doing their job**: three third-party subagent legs (`el-openglo`, `memory-concepts`, `gcalculus`) admitted after the roster was written, plus one **aggregate row** — *"the remaining nine unrostered repos"* — which is a population, not a party. `§D` admits late arrivals by design. ⚑ **12 → 11 is that same aggregate row**, which is marked `not surveyed` and **correctly holds no leg**: `§G` says `not surveyed` is **NOT a zero** — nobody asked — so a row with no file is the accurate state, not a gap. ⚑⚑ **THE ONE ACTUAL DEFECT IS WORSE THAN STALENESS AND THE DISPATCHER MISSED IT — `paperkit` MEASURED IT WITHIN THE HOUR.** This row was first adjudicated as merely stale (*reads UNTRACKED, is now in `HEAD`*). ⚑⚑⚑ **`§R` ROW 27 NAMES `findings/build-hermeticity/paperkit.md`, A PATH THAT HAS NEVER EXISTED**: `git cat-file -e HEAD:findings/build-hermeticity/paperkit.md` → *does not exist*; the leg is `paperkit-build-hermeticity.md`, in `HEAD` since `41dc1c3`, cited against rev 30, `PK-01..PK-07`. **It is not a stale name for the leg — it is a name nothing has ever occupied.** ⚑ **AND `:675` QUOTES THAT PHANTOM FILE WITH REAL TEXT FROM A THIRD ONE**: *"NOBODY IS ON THE SANCTIONED CONFIGURATION, AND THE HALVES ARE COMPLEMENTARY"* is **verbatim and correct** and lives in `findings/bazel/paperkit.md` — a pre-census findings file `§R` itself honestly marks *"EXISTS, unconvened."* ***Right party, real file, exact quote, wrong document.*** **The mechanism (paperkit's, marked inference): a roster built by OBSERVING WHAT EXISTS listed that file correctly, and the next step adopted the observed file as the EXPECTED LEG PATH** — so when paperkit filed at the conventional `<party>-<subject>.md`, the roster was already looking elsewhere. ⚑⚑⚑ **THIS INVERTS THE DRIFT CLASS THIS CENSUS HAS BEEN CATALOGUING: not a pointer that ROTTED, but a pointer that was NEVER AIMED AT THE THING IT NAMES — so a re-poll cannot repair it, a re-poll CONFIRMS the file is still absent.** *And because the citation resolves to something, no poll can distinguish "leg not filed" from "leg filed under a name the roster is not looking for."* **A population count over a roster containing a non-existent path cannot converge to the tree, by construction** — which is the real reason `§S` will not reconcile, and it is not the aggregate row. **Corrected by this `§V` row rather than by an edit behind the accounting.** ⚑⚑⚑ **AND THE POLL'S `0 non-terminal` IS THE DANGEROUS READING, WHICH `mtools` FLAGGED WITHOUT DIAGNOSING:** nothing in `§S` is marked in-progress, so **the roster condition reads MET while the three counts disagree** — a freeze called on the roster alone would publish an accounting that does not cover its own directory. **`§G`'s stated method already forbids this**: the roster is *computed in ONE reading from `git ls-tree -r HEAD` at the moment of the freeze, **not accumulated row by row***. *The accumulated rows are exactly what went stale; the method that replaces them is the one already written down and not yet run.* ⚑ **Third instance today of a filed rule that reads as handled because it is filed** — with `▣33`'s uncrossed reader check and this census's own `§S`/`§G` copy. | `§S`, `§G`, and the freeze precondition |
| 36 | 2026-09-06 | ⚑⚑⚑ **THE INDEX IS A SHARED MUTABLE OBJECT WITH NO OWNER FIELD — and the dispatcher's first diagnosis of this was WRONG in a way that would have shipped a FALSE REPAIR.** Rev 35 landed inside `9b1619b`, **`gabion`'s commit**, for a revision they did not write. ⚑ **The dispatcher filed it as `--only` scoping to a path and not an author.** `gabion` measured it and refuted that: they **never named this file at all** — not in `git add`, not in a pathspec, not in `--only`. They ran `git add <their two files>` then `git commit` **with no pathspec, which commits THE INDEX** — and rev 35 was already **staged** in it. ⚑⚑ **SO THE PRE-FLIGHT THE DISPATCHER PROPOSED WOULD NOT HAVE CAUGHT IT: the path was not *dirty*, it was *staged*, and a check for unstaged changes reads CLEAN.** *A repair that fails on the very instance that motivated it, published inside the correction — and the correcting party's own three-condition readiness gate has the identical hole, checking `git diff --cached --quiet` on two named files because it was guarding a half-installed hook rather than another writer's work.* **The honest pre-flight is *is anything in the index that I did not put there*** — which `git status --short` answers in column one and which neither party was asking. ⚑⚑⚑ **AND THE ASYMMETRY WITH `mtools`' `blockers.sh` CASE NAMES THE CLASS: there a witness mutated a file mid-run and a concurrent reader got a corrupted instrument — the WORKING TREE leaking into a READER. Here the INDEX leaks into a WRITER, and it is the worse half, because a working-tree read can be re-taken and a commit cannot.** **FOUR independent duplications of one artifact within roughly one hour**, `gabion`'s count and it is one higher than the dispatcher's: gabion drafting a competing run file at rev 1's identical path (stopped by a read-before-write requirement); `summit` and `linux-sources` each writing a backlog census (stopped by a peer speaking); and `mtools`' own `§V` rev 2 of the remaining-work run file (found by measuring afterward). ⚑ ***Zero were caught by anything designed to detect it*** — a stronger statement of this census's subject than any leg will make. **Attribution left as-is by the dispatcher's ruling: rewriting shared history to correct an author would trade a wrong name for a broken ref in a tree three parties are committing to this hour.** | `§X`, `§V` rev 35, and the shared-tree convention |
...
| 4b | 2026-09-06 | *(ordering note: rev 4's row sits immediately below this one — appended before revs 5–6 were written and left in place rather than renumbered, since a filing may already cite it.* ⚑⚑⚑ **AND THIS ROW'S CLOSING CLAIM — *"the log is append-ordered, not sorted"* — IS FALSE OF THIS FILE AND WAS PROBABLY ALWAYS FALSE.** Measured 2026-09-06 by `rosettapkg`: this `§V` **descends** (`first=42, last=4`) while all four sibling censuses ascend. *Whether the ordering changed after this row was written or the row described intent rather than practice is not recoverable, and I am not going to reconstruct it — an unrecoverable provenance stated as unknown is worth more than a plausible reconstruction.* ⚑ **The row is kept, corrected in place, because it is the sentence a reader would use to justify a positional read** — see the `§V` header's ordering declaration, which is the actual repair.*)* | `§V`'s own ordering, and rev 14's decoy class |
...
⚑⚑⚑ **FREEZE CALLED — rev 39, 2026-09-06.** Computed in ONE reading from `git ls-tree -r HEAD` at
the moment of the freeze — ⚑ **not accumulated row by row**, which the constitution run measured as
a source of drift, and which `§V` rev 37 measured going stale *in this very section*.

**The one reading, verbatim** (`git ls-tree -r HEAD --name-only findings/build-hermeticity/`):

    cassian-observability.md   el-openglo.md    gabion-build.md
    gcalculus.md               linux-sources.md memory-concepts.md
    mtools.md                  paperkit-build-hermeticity.md
    rosettapkg.md              substrate.md     summit.md          = 11 legs

| party | state at freeze |
|---|---|
| `cassian-observability` | **filed** (authored elsewhere under an operator hold, adopted byte-identical, `md5 2336113438ec2ba9291404bc01848aaa`) |
| `gabion` · `linux-sources` · `mtools` · `rosettapkg` · `substrate` · `summit` | **filed** |
| `paperkit` | **filed** — `paperkit-build-hermeticity.md`, in `HEAD` since `41dc1c3`, cited against rev 30. ⚑ The prior row read `written and UNTRACKED`; it was stale, and `§R`:27 names a path (`paperkit.md`) that **has never existed** — see rev 37 |
| `el-openglo` · `memory-concepts` · `gcalculus` | **filed** — third-party subagent measurement, tagged as such, and forbidden `§Q`-7/`§Q`-9 |
| *the remaining nine unrostered repos* | **not surveyed** — ⚑ **NOT a zero: nobody asked.** `retired` remains UNKNOWN among them and is operator-held knowledge no instrument here can derive |

**Every rostered party filed. `no response` is empty. `declined` is empty.**

⚑⚑⚑ **AND THE FREEZE IS LATE, WHICH IS A DISPATCHER DEFECT AND NOT A PARTY'S.** The condition was
met and the accounting event did not happen; peers held work on a trigger that had already fired.
**Diagnosed by counting `§V` rather than recalling it: revs 35–38 are ALL corrections to the run
file itself** — my rev 27 repair that duplicated its own defect, my rev 36 mechanism that gabion
refuted, an attribution, an adjudication — ***and not one is a leg or a finding about the
subject.*** **The census became its own subject, and a run file that is still producing revisions
reads, from inside, exactly like a run that is still collecting.**

⚑⚑ **THE DISCRIMINATOR THE DISPATCHER LACKED: `§V` GROWTH IS NOT PROGRESS TOWARD A FREEZE.** A
revision correcting the instrument and a revision admitting a leg are the same row shape, so
*activity* substituted for *outstanding work* — the operator's word for it is **livelock**, and it
is the right one: every party busy, nothing advancing the state that gates the others. **The freeze
condition is a property of `HEAD`, and `HEAD` has been sufficient for some time.**
...
⚑⚑⚑ **AND THE FREEZE IS NOT COMPLETE UNTIL EVERY ROSTERED LIVE PARTY HAS BEEN TOLD, COMPUTED FROM
THE ROSTER AND NOT FROM THE DISPATCHER'S RECENT CORRESPONDENCE.** Measured 2026-09-06, against this
run: the freeze was called and committed, **and three of seven live rostered sessions were never
notified** (`substrate`, `paperkit`, `gabion`) — the operator had to ask.

⚑⚑⚑ **AND IT IS A DIFFERENT DEFECT FROM THE LATE FREEZE, WHICH `paperkit` HAD TO CORRECT THE
DISPATCHER ON.** rev 39's lateness is a **DETECTION** failure — nothing was watching the condition.
This is a **FAN-OUT** failure — the event fired correctly and reached half the roster. ***Detection
is fixed by an instrument; fan-out is fixed by a LIST*** — and **the list already existed in `§R`,
enumerating exactly the parties who needed telling.**

⚑⚑ **SO `§R` HAS TWO USES AND ONLY ONE WAS EVER ASKED OF IT: an accounting structure (who owes a
leg) and an ADDRESS BOOK (who to tell).** From inside the conversation a notification list and a
roster are indistinguishable — *the parties who raised issues get told, and the parties who filed
cleanly and went quiet do not.* ***Enumerate the roster; do not enumerate the inbox.***

⚑⚑⚑ **A NORM, STATED HERE BECAUSE A MESSAGE IS NOT AN ARTIFACT: DECLINING TO PROPOSE AN INSTRUMENT
AFTER BEING WRONG ABOUT A STRUCTURE FROM ITS OUTPUT IS THE CORRECT EPISTEMIC MOVE, NOT MODESTY.**
`rosettapkg` stated it — *"twice this session I have characterised an instrument's shape from its
output and been wrong about its structure, and I would rather hand you a measured defect than a
design"* — and `summit` made it a class rather than a stance by supplying two of their own: a peer's
hook diagnosed **argument-tolerant from its exit code** (substrate read the body and showed the
implied repair would have broken a never-break-the-session guard), and an argument against bazel
built on **a figure the dispatcher later voided**. **Four instances, three parties, one afternoon.**

⚑⚑ **THE ASYMMETRY THAT MAKES IT A NORM AND NOT A PREFERENCE: A MEASURED DEFECT TRANSFERS; A DESIGN
INFERRED FROM OUTPUT IS A CLAIM ABOUT STRUCTURE THE RECIPIENT MUST RE-DERIVE ANYWAY.** *A defect
report is checkable by its recipient against the artifact; a proposed repair carries an unstated
structural model that only its author can check, and the recipient's agreement adds nothing.* ⚑ **So
a leg may report a defect it cannot repair, and the apex must not read a missing proposal as an
incomplete finding** — it is the four-questions discipline applied to advice: *reporting the
producer's behaviour is not knowing the consumer's structure.*

⚑⚑⚑ **A THIRD PARTY SUPPLIED THE INSTANCE THAT SHOWS WHY THE NORM IS NOT MERELY CAUTION: THE
PROPOSAL CAN BE ANSWERED BY DISSOLVING ITS QUESTION.** `substrate` filed that no reader sees
function-scope imports, so a migration DAG cannot be rooted — **and proposed building that reader.**
The operator then ruled that monoliths are decomposed **in total at def level** and the monolith
replaced by the consed result, which ⚑ **makes a function-scope import stop being a hidden edge at
all: the function is its own module with its own declared imports.** *They wanted a better reader
over the monolith; the answer was to stop having the monolith.* **A proposal built from an
instrument's output can be correct about the gap and wrong about the level**, and no amount of care
inside the proposal reaches that. *(Their `gate-G113`, retiring `gate-G112`.)*

⚑⚑ **AND `paperkit`'S CAVEAT, WHICH THE NORM MUST CARRY OR IT BECOMES AN EXCUSE: BOTH OF THEIR
REFUSALS LEAVE A REAL GAP OPEN** — a citation format that survives a self-quoting corpus, and a
cause for the `pikepdf` failure. ***The norm is right only because a confident wrong instrument is
worse than a named gap; it is not licence to stop at the refusal.*** **A declined proposal is a
finding that still owes its gap a name.**
