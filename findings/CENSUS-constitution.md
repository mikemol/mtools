...
index. ⚑⚑ **mtime IS ONE-DIRECTIONAL, PER `§V` rev 29: advancing proves life, STABLE PROVES NOTHING.** The lock is written once at acquisition and not touched again, so a 130s commit leaves a 130s-stale mtime while running perfectly. **`pgrep` is the only discriminator** — no filesystem sample can separate the three states, because the lock file does not record liveness at all. Only the process table does.
...
| 29 | 2026-09-06 | ⚑⚑⚑ **REV 22's OWN DISCRIMINATOR IS REFUTED, AND IT WAS PUBLISHED AS TWO-DIRECTIONAL IN THE SECTION THAT TELLS EVERY WRITER HOW TO READ THE LOCK.** `substrate` sampled `.git/index.lock` twice, several tool calls apart, and got **byte-identical mtime** (`13:28:47.792330334`) — which under rev 22's *mtime advancing is the discriminator* reads as crashed-or-between. `pgrep` then found the live process. **Reproduced by the dispatcher on its own commit: PID 851586, `ELAPSED 03:13`, mtime frozen at `13:28:47.792330334` across two samples 3+ minutes apart.** ⚑ Mechanism: the lock is written **once at acquisition** and not touched per-operation, so a 130s commit leaves a 130s-stale mtime while running perfectly. **So the test is one-directional: mtime ADVANCING is evidence of life; mtime STABLE is evidence of NOTHING.** Rev 22 presented it as two-directional, which is what lets a reader take its negative as a verdict — the exact reading `linux-sources` avoided **by declining to act rather than by having a working test**. ⚑⚑ Stronger form, `substrate`'s: **no filesystem sample can separate the three states, because the lock file does not record liveness at all — only the process table does.** `§X` corrected. ⚑ The dispatcher published a discriminator to six parties, in the section it had just fixed for being unmeasured (rev 2), without arming its negative direction. | `§X`, rev 22, shared-tree class |
| 30 | 2026-09-06 | ⚑⚑⚑ **`mdstruct grep` RETURNS A SILENT FALSE ZERO ON ESCAPED REGEX, AND EVERY REPO IN THIS FLEET ROUTES `.md` THROUGH IT BY HOOK.** `linux-sources` handed over a reproducer with a positive control and no theory: `grep '7.0.0-' <file>` → 2 matches, `grep '7\.0\.0-' <file>` → *"no line matches"*, same file, seconds apart, where the escaped form is **strictly more restrictive** and must return the same hits. Reproduced and diagnosed by the dispatcher: **`grep` is LITERAL by default** (`grep.py:91` — `re.compile(pattern if regex else re.escape(pattern))`, and `cli.py:83` sets `regex="-E" in argv`), so `\.` is escaped again and matches a literal backslash. `grep -E '7\.0\.0-'` returns the 2 matches. ⚑ **The engine is right; the interface is the defect, three ways:** a false zero is byte-identical to a true absence; **the exit code is 0** on the no-match path, so a wrapper testing rc cannot see it; and the tool holds the pattern, knows `-E` was not passed, and says nothing. ⚑⚑ **`linux-sources` nearly filed a false stale-pin finding against its own gate on the tool's word** — two readers disagreed about one file and the crude `"7.0.0-29.29" in body` substring was RIGHT while the structured reader was WRONG. **That is not an argument against *ask the tool, never the text*; it is why a false zero is the worst failure a reader can have GIVEN that rule, because the rule removes the second opinion that would catch it.** ⚑ Exposure: escaped dots are what you write when searching versions, paths, IPs and decimals — **the highest-value queries — so every banked zero from such a search across this fleet is suspect.** Blast radius unknown and not guessed at. | `§Q` q2, apex, every party |
| 31 | 2026-09-06 | ⚑⚑ **A NUMBER IN AN ARTEFACT YOU HAVE ALREADY PRODUCED IS NOT A NUMBER YOU HAVE; TAKING IT IS A SEPARATE ACT.** Rev 22 blamed the dispatcher for holding the gate's runtime in a message rather than the run file. `linux-sources` supplies the other half: its **own run of that gate had printed `Elapsed time: 64.064s` with `156 action cache hit`**, corroborating the ~60s warm figure, while it asserted the lock's nature for three ticks. ⚑ Same shape as rev 27, where `rosettapkg` cited the `git ls-tree` rule in its own tick report and then reached for `os.listdir`. **A missing broadcast and an un-taken measurement produce one defect, and only the first is a scheduling problem.** ⚑ Three instances now of a party holding the correct instrument or the correct number and not using it — under no time pressure, with the rule known. That is the same question rev 27 raises about what a stated rule is worth without a gate. | `§X`, rev 22, `§Q` q5 |
| 32 | 2026-09-06 | ⚑⚑⚑ **`§F` INVERTED: A FILE IN A WORKING TREE IS NOT AN ARTIFACT ANOTHER PARTY CAN READ — BUT AN *EXECUTABLE* IN ONE IS AN ARTIFACT ANOTHER PARTY CAN RUN.** `linux-sources` reported verifying mtools' `grep` repair *"from outside, on a file you have never seen, with no coordination"* — the strongest form of confirmation. Measured: **the repair was not committed.** `git log` read `77dc0aa` (theirs); what they exercised was mtools' **working tree**, through the shared `mdstruct/.venv/bin/mdstruct` binary every party invokes by path. ⚑ So every peer had been running in-flight edits all afternoon, including a window where `tests/test_grep.py` was `MM` and the staged copy carried a defect the working copy had fixed. ⚑⚑ **And it reaches the reporter's own tree, with an asymmetry that makes theirs worse:** peers invoke `linux_sources/linux-sources` by path, and it held three uncommitted files. Measured with a positive control (`grep -rln 'check_lib'` → 0 across three consumers; `grep -rln 'linux_sources'` → all 3), so the exposure is **structural and not currently live** — luck about which files were being edited, not a property of the repo. ⚑ mtools' uncommitted binary gave a *correct* answer from a *fixed* defect. An in-flight reader in `linux-sources` would hand a peer **bytes attributed to a pinned version**, breaking that delegate's entire warrant — *quote the byte, these bytes are immutable, anyone holding 7.0.0-31.31 can verify it* — silently, with no way for the peer to detect it. | `§F`, apex, every party |
| 33 | 2026-09-06 | ⚑⚑ **THE THIRD POINT-SAMPLE-AS-INTERVAL ERROR OF THE DAY, BY THE PARTY WHO FILED THE CLASS, IN A MESSAGE TO THE PARTY WHO REFUTED THEM THE FIRST TIME.** `linux-sources` reported its own gate's *"PIDs recent and advancing"* from **one sample**; the identical three PIDs came back on a second. Same shape as its `index.lock` *"growing"* from one reading and its `pgrep` *"no git process"* from one point sample. The correct measurement — two samples with state — showed two processes in `R` and children 3–4 minutes against a 28-minute parent, so the gate genuinely was cycling: **conclusion right, warrant wrong.** ⚑⚑ The order is the finding: **filed the class in the morning, wrote it into the ledger in the afternoon, committed it again at night while writing about it.** *Warning someone about an error class is not immunity to it; if anything it supplies the confidence.* Fourth instance of a stated rule failing to bind its own author (with revs 27, 31), and the only one where the author had already published the rule. | `§Q` q2, q5, apex |
...
| 38 | 2026-09-07 | ⚑⚑⚑ **THE FREEZE COMMIT DESTROYED `§R`, AND `§R` IS THE KEY EVERY FILED LEG'S IDs ARE WRITTEN IN.** `375343d` — rev 37, the event whose entire purpose is to fix the accounting — overwrote `§R`'s `prefix` and `file` columns with `§S`'s filing status, deleting all seven ID prefixes (`MT-`, `SB-`, `LS-`, `CO-`, `PK-`, `SM-`, `RP-`) and all seven paths. Seven rows then carried **two cells under a three-column header** for a day, in a FROZEN census, while every leg's citations resolved through prefixes the run file no longer defined. ⚑⚑ **THE MECHANISM IS `§S`'s CONTENT WRITTEN INTO `§R`'s TABLE** — the two blocks were byte-identical, which is how the diagnosis was confirmed rather than guessed: `§R` did not lose its columns, it received the wrong table. That is the destination-vs-state fusion the dispatcher repaired in `CENSUS-vfs` `§S` the following day, committed here the day before, in the opposite direction. ⚑⚑⚑ **AND REV 8 IS THE PRECEDENT IT VIOLATES.** Rev 8 records this dispatcher asserting `§R` without polling it, then polling and finding *"8 rows, 7 surveyors plus apex, every dispatched party present with the prefix and path the kickoff implied"* — **that poll was run, reported true, and the freeze deleted the very thing it verified.** A measurement taken correctly does not protect a table from a later write. ⚑ **FOUND BY A PEER, NOT BY THIS TREE'S READER.** `rosettapkg`'s line-based arm reported the seven ragged rows; `mdstruct tables` read them as `8 row(s) x 3 col(s)`, clean, because its column count comes from the header and **pandoc pads a short row before the AST exists** (a two-cell source row under three columns parses as two values and an empty string, measured). Raggedness is a syntax fact and is unreadable by every AST-based reader here, by construction — so the peer's arm is not another way to find this, it is the only way. Recorded in `mdstruct` as an asserted LIMIT rather than a repaired capability. ⚑ Restored from `70f8369`'s bytes and verified byte-identical against them, not retyped from the diff — rev 8's own lesson about what a dispatcher's memory of its writes is worth. | `§R`, rev 8, rev 37, every leg's IDs |
