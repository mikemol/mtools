...
| 16 | 2026-09-06 | ⚑⚑ **TWO `mdstruct` IMPLEMENTATIONS; THE COLUMN-SCOPED PREDICATE IS IN THE UNREACHABLE ONE.** `mtools-ce` built `rows --col N --starts TEXT` in `mtools/mdstruct/`; the reader `hook_structural_query` mandates (`substrate/scratch/mdstruct.py`) has no such mode — measured, exit 2. **`§G` keeps the `--where` + count-identity poll**, which runs on the sanctioned reader. ⚑ Adopting the better mode would point every leg at something its own routing cannot invoke. **This is `§Q`-8 generated by the survey's own instrument, mid-survey.** | `§G`'s predicate; apex `§Q`-8 |
...
⚑ **So the run keeps the `--where` + count-identity poll above**, which works on the reader the hook
mandates. The column-scoped form is **better and unavailable**; adopting it in `§G` would point every
leg at a mode its sanctioned reader does not have — a routing failure dressed as an upgrade.
