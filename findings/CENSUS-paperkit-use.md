...
| `summit` | **filed, not in HEAD** — `summit:proceedings/census-paperkit-use-summit-leg.md`, 12,799 bytes, staged; `git cat-file -e HEAD:…` → 128, verified by the dispatcher 2026-09-19. The acceptance is honoured (their leg carries the finding that it was never externalized, SM-7). Their commit gate is held by an operator ruling and a dead venv; not theirs to override |
...
| `rosettapkg` | **filed, not in HEAD** — `rosettapkg:census/paperkit-use-leg.md`, 134 lines, staged 2026-09-19; `git cat-file -e HEAD:census/paperkit-use-leg.md` → 128. ⚑ Not in HEAD because rosettapkg's `cite-check` is correctly refusing every commit while `linux-sources`' reader is dirty on its path — the dispatcher's own leg sits in the state the dispatcher named for summit one revision ago |
