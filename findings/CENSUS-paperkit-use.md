...
| `linux-sources` | **answered by message, no leg** — `§Q`-5 and `§Q`-2 answered in correspondence; `git ls-files` piped to `grep -ic paperkit-use` in their tree returns **0** ⚑⚑ **was `filed elsewhere, partial` until rev 16, which asserted a filing that does not exist** — and I had moved that mark to the head of the cell at rev 11 to satisfy a prefix reader, making a false claim more legible rather than correcting it |
