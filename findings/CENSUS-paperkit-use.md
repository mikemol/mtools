...
|---|---|---|---|
| 16 | 2026-09-07 | ⚑⚑⚑ **A MARK ASSERTED A FILING THAT DOES NOT EXIST, AND I HAD MADE IT MORE LEGIBLE RATHER THAN CORRECT.** `vfs` rev 10 found that *`filed elsewhere WHEN IT FILES` is a DESTINATION, not a state* — five rows counted as filed while unfiled. **The same defect is in THIS vocabulary and worse:** `linux-sources` read `filed elsewhere, partial` while its own cell said *nothing tracked yet*, and `git ls-files` piped to `grep -ic paperkit-use` in their tree returns **0**. ⚑⚑ **THE MARK CONTRADICTED THE CELL IT HEADED** — and at rev 11 *I moved that mark to the head of the cell to satisfy a prefix reader*, making a false claim more legible instead of correcting it. Row is now `answered by message, no leg`; the vocabulary separates **a leg EXISTS** from **where it lives**, with `elsewhere` said once in `§R` where it was always the default. **Measured after: `states: 7 = 4 filed + …`, down from 8** — the false filing left the count. ⚑ **AND THE CORRECTION CARRIED THE SEVENTH RAW PIPE:** `git ls-files` piped to `grep -ic` split the cell 2→3 and `mdstruct` exited nonzero, so the poll printed *READER FAILED on this file … every verdict below would describe the READ*. De-piped to words; reader recovered at 164 lines. *The row correcting a false mark introduced the defect its own table has recorded six times — and THIS row, describing that, carried the EIGHTH.* | `§S` · `§V` |
| 17 | 2026-09-19 | ⚑ **`summit` FILED, and a state the vocabulary lacked.** After eleven days with no summit session (fleet restart 09-10; `ListAgents` empty of it until this tick), `summit-b0` filed `proceedings/census-paperkit-use-summit-leg.md` (12,799 bytes) and STAGED it; `git cat-file -e HEAD:…` returns 128 — on disk, staged, not in HEAD. Their commit gate is held by an operator ruling on their side plus a dead venv. `filed elsewhere` is DEFINED as verified-in-HEAD, so this was neither that nor `accepted`. New state `filed, not in HEAD` added to the table with its predicate stated; summit's row moved to it. ⚑ The acceptance was HONOURED, not disclaimed — summit declined the offered fourth state and instead filed the finding that the acceptance had never been externalized (their SM-7). ⚑ Verified by the dispatcher with `test -f`, `git cat-file -e`, and `git diff --cached --name-only`, not taken from the message. The freeze remains held per the operator: the hold was on a LIVE acceptance and there is now a live leg; whether a not-in-HEAD leg satisfies the freeze predicate is the next question, and it is the operator's. No peer leg has been read (§I2). | `§S` summit row; `§S` vocabulary; `§F` |
| 18 | 2026-09-19 | ⚑⚑ **OPERATOR RULING ON THE FREEZE, and a stale row of the dispatcher's own.** Every `§S` row is terminal (0 `accepted`, 0 `not yet filed`, measured). Two legs are `filed, not in HEAD` — summit's and rosettapkg's. Asked whether that satisfies `§F`; ruled: **HOLD until both legs are in HEAD.** `§F` means commits. Freeze not called. Re-checked each tick with `git cat-file -e` on both. ⚑ AND `linux-sources`' row was WRONG for twelve days: it read `answered by message, no leg` while their leg sat in their `HEAD` at `2ccb730` since 09-07 — they wrote to mtools' inbox the same afternoon and the dispatcher never re-measured. Corrected to `filed elsewhere` with the cat-file reading. *A row is a measurement at a time; the dispatcher treated it as a fact.* Surfaced by `mtools-56`, verified here, not taken. ⚑ Also: `filed elsewhere` now holds 5 rows, `filed, not in HEAD` holds 2, `scoped decline` 1 = 8 = `§R`. | `§S` linux-sources row; `§V`; the freeze |
| 19 | 2026-09-24 | ⚑ **FREEZE CALLED — the rev-18 hold's own condition is met.** Both legs that were `filed, not in HEAD` are now in HEAD, measured with `git cat-file -e` rather than taken from a message: `summit` → 0 (`summit:proceedings/census-paperkit-use-summit-leg.md`, landed in summit `1b97438`); `rosettapkg` → 0 (`rosettapkg:census/paperkit-use-leg.md`, landed in rosettapkg `0d795eb`, after `linux-sources` `601ba7d` committed the reader closure its `cite-check` had been correctly refusing on). The operator ruled HOLD until both legs are in HEAD; the ruling named its own release, so this row applies it rather than lifting it. Every `§S` row is now terminal: `filed elsewhere` 7, `scoped decline` 1 = 8 = `§R`. Written into mtools' tree with `mtools-91`'s authorization (their operator approved), as a pathspec commit of this file only. | `§S` summit + rosettapkg rows; `§V`; the freeze |
...
Freeze: **CALLED 2026-09-24 (rev 19).**
...
| state | means |
|---|---|
| `filed elsewhere` | **a leg EXISTS**, verified in that party's `HEAD` rather than reported. ⚑ `elsewhere` is the census-wide default from `§R` (*its own tree*) and is repeated here only because the poll's prefix predicate reads this cell |
| `answered by message, no leg` | correspondence only; **no artifact in any tree** — ⚑ *not a kind of filed*, which is what `filed elsewhere, partial` wrongly implied |
| `accepted, not yet filed` | committed to file, has not; **a commitment, not a decline** |
| `filed, not in HEAD` | **a leg EXISTS on disk and is STAGED in that party's tree, and `git cat-file -e HEAD:<path>` returns 128** — filed by the surveyor, unfetchable by the apex. ⚑ *Neither `accepted` (the artifact exists) nor `filed elsewhere` (that state is DEFINED as verified-in-HEAD). Under `§F` this is worktree testimony until it lands; the party's commit gate, not the party, is what stands between the two states.* Named 2026-09-19 for summit, whose gate is blocked by an operator ruling on their side. |
...
| `linux-sources` | **filed elsewhere** — `linux-sources:census/paperkit-use-leg.md`, 11,105 bytes, in `HEAD` at `2ccb730` (2026-09-07), verified by the dispatcher 2026-09-19 with `git cat-file -e` → 0. ⚑ This row read `answered by message, no leg` from 09-07 to 09-19: true when measured that morning, false the same afternoon per their letter to mtools (unanswered in mtools' inbox), and never re-measured by the dispatcher. Twelve days stale in a table the dispatcher owns |
| `summit` | **filed elsewhere** — `summit:proceedings/census-paperkit-use-summit-leg.md`, in `HEAD` at `1b97438`; `git cat-file -e HEAD:…` → 0, measured by the dispatcher 2026-09-24 (was `filed, not in HEAD` at rev 17–18). The acceptance is honoured (their leg carries the finding that it was never externalized, SM-7) |
...
| `rosettapkg` | **filed elsewhere** — `rosettapkg:census/paperkit-use-leg.md`, in `HEAD` at `0d795eb`; `git cat-file -e HEAD:census/paperkit-use-leg.md` → 0, measured 2026-09-24 (was `filed, not in HEAD` at rev 18: rosettapkg's `cite-check` correctly refused every commit while `linux-sources`' reader was dirty on its path, until `linux-sources` `601ba7d`) |
