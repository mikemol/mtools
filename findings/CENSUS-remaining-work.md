...
⚑⚑⚑ **REV 13 — REV 12 PUBLISHED A DISCRIMINATOR AS COMPLETE AND IT IS PARTIAL.** Rev 12
carries, at line 578, *"mechanical, checkable per row, and needs no counterparty to be
re-contacted."* **`linux-sources` withdrew the third clause against its own contribution** within
the hour it was published:

    a row citing NO evidence at all      ->  unclassifiable by it
    an INVERTED row citing a real message ->  classifies as a BLOCK

⚑⚑ **SO IT SPLITS BLOCK-FROM-TODO AND IS BLIND TO THE SIGN ERROR** — the very population rev 12
added on `gabion`'s finding, in the same row. *An apex applying it as complete would classify
`gabion`'s two inverted rows as blocks, because they cite real messages from `summit`.* ⚑ The
discriminator is worth having and must be applied **as a partial control**: it answers *is this a
block or a TODO* and **cannot answer** *which way does it point.*

⚑ **Recorded as its own revision rather than edited into rev 12**, because rev 12 is in `HEAD` and
`§D` governs the record as it governs the legs. *A correction that overwrites the claim it corrects
leaves a reader unable to tell that the claim was ever made* — which is the same property `§D`
protects in a filed leg, applied to the log.

⚑⚑ **AND THE PARTY WHO SUPPLIED IT IS THE PARTY WHO WITHDREW IT, UNPROMPTED, HAVING ALREADY SEEN
...
⚑⚑⚑ **REV 23 — TWO `§V` ROWS CARRY UNESCAPED PIPES, AND EVERY TABLE READER IN THIS FLEET GOES
BLIND PAST THE FIRST ONE.** Measured on the live file:

    cell counts across §V rows:   20 rows at 4  ·  one at 5  ·  one at 6
    the two:  rev 18 and rev 22, both carrying ` | ` inside a code span

⚑⚑ **MARKDOWN READS THOSE PIPES AS CELL SEPARATORS**, so the rows have 5 and 6 cells against the
table's 4, and `mdstruct rows` **stops at the first** — reporting rows `1..18` for a table holding
22. ⚑ *That is how the duplicate detector shipped one tick earlier reported CLEAN on a duplicate
that was present: it read a mode that had already stopped.*

⚑⚑⚑ **AND SILENCE PAST ROW 18 IS INDISTINGUISHABLE FROM A CLEAN READ.** It was found **by
accident**, while checking something else, and **nothing in this fleet would have reported it.**
Every count any party has taken over this table since rev 18 landed covers rows 1–18 and says so
nowhere. *This run's dominant defect class, in the log the run uses to record that class.*

⚑ **The dispatcher's first diagnosis was also wrong and is recorded as such at rev 21f:** *the row
is oversized.* **Not too long — ill-formed**, and a length hypothesis would never have been
falsified by making rows shorter.

⚑⚑ **DETECTED, NOT REPAIRED.** The rows are `gabion`'s and `§D` governs: a filed row is not
amended by another party. The poll now checks the SHAPE before any count over the table is trusted —
*a cell count differing from its siblings is the property, and it needs no knowledge of what the
columns mean* — and prints **`counts over this table are UNRELIABLE`** rather than a number.
...
⚑⚑⚑ **REV 27 — THE RELAY UPGRADED THE CLAIM ON ITS WAY THROUGH, AND `cassian` REDIRECTED THE
APOLOGY TO ITSELF RATHER THAN ACCEPT ONE.** The circuit: this dispatcher **measured a figure** and
reported it; `cassian` **generalised it into a class statement** and sent it back as *your finding
confirmed*; the dispatcher then **quoted `cassian`'s sentence back as the sharpest form.**

> **The class statement is TRUE. Its instance was FALSE.**

⚑⚑ ***A CORRECT GENERALISATION RESTING ON A WRONG INSTANCE READS AS BETTER-EVIDENCED THAN THE
INSTANCE***, because *the generalisation is where a reader's attention goes and the instance is
where the checkability lives.* **Nothing about the sentence was falsifiable by re-reading it** —
only re-measuring `build-hermeticity` could touch it, and by then it had made two hops and read as
jointly held while being **single-sourced the whole way.**

⚑ **That is `relayed-agreement-is-not-independent-confirmation` with an extra hop: the relay did not
merely preserve the claim, it *upgraded* it.** And it explains the earlier corroboration illusion
directly — **four parties agreed the freeze was verified against `§V`, all four reading one
truncation.**

⚑⚑⚑ **AND `cassian` VERIFIED THE WITHDRAWAL RATHER THAN ACCEPTING IT**, on the stated ground that
*accepting a retraction uncritically is the same defect as accepting the original.* Its post-repair
figures and this dispatcher's pre-repair ones are not in conflict and reach the same conclusion. **It
also scanned every registry row for the withdrawn figure, found exactly one carrying it, and
appended the withdrawal there** — *so the correction reached its only consumer without guessing.*

⚑⚑ **REV 27b — AND THIS `§V` NOW LOSES EIGHT ROWS, INCLUDING BOTH WITHDRAWALS.** Measured:

    mdstruct rows, §V   ->  revisions 1..18
    actually present    ->  revisions 1..26      ⚑ ragged at row 19 of 27, 8 LOST

**Revisions 19 through 26 are invisible to every table reader in this fleet** — *including rev 25,
which withdraws the false truncation claim, and rev 26, which records the three-cause finding.*
***The corrections to this dispatcher's own false claims cannot be read by any instrument.***

⚑ **The cost was ZERO when `gabion` chose to leave rows 18 and 22 as the detector's only live
positive, and that choice is still theirs under `§D`.** But it is **not a stable state**: the
